...
commits_2012 <- read_excel("~/Desktop/Sustainable_Vision/sustainable_vision_grants_2012_proposals.xlsx") %>% 
  ...
  select(-ID.x, -ID.y) %>% 
  left_join(match_c) %>% 
  write_csv("new/2012/commits_2012.csv")
...
membership_2012_1b <- proposal_2012 %>% 
  select(NAME, ZENN_ID__C) %>% 
  rename(
    "TEAM_NAME_TEXT_ONLY_HIDDEN__C" = "NAME"
  ) 

membership_2012_2 <- merge(membership_2012_1a, membership_2012_1b) # add ZENN ID

advisors <- read_excel("~/Desktop/Sustainable_Vision/sustainable_vision_grants_2012_advisors.xlsx") %>% 
  rename(
    "ZENN_ID__C" = "Zenn ID"
  )

membership_2012 <- merge(membership_2012_2, advisors) %>% 
  mutate(
...
task_2012 <- read_excel("/Volumes/GoogleDrive/My Drive/Sustainable_Vision/sustainable_vision_grants_2012_post_award_notes.xlsx", 
  ...
  write_csv("new/note_task_2012.csv")
